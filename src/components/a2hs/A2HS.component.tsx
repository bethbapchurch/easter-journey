...
const { A2HS: A2HSWrapper, A2HSControls, button } = styles;
...
export default class A2HS extends PureComponent<unknown, InstallState> {
  ...
  dismissNotification = (): void => {
    ...
    browserStorage.setItem(A2HS_IDENTIFIER, true, NOTIFICATION_IGNORE_TIME);
  };
  ...
  install = (): void => {
    ...
    self.pwaInstallPrompt.userChoice.then((choice) => {
    ...
    return (
      <figure className={A2HSWrapper}>
        ...
        {isMobile.iOS() ? (
          ...
          <button onClick={this.install} className={button}>
            Add to Home Screen
          </button>
        )}
