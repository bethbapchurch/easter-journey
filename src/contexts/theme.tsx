...
export type ThemeValue = {
  isDarkMode: boolean;
  userTheme: [boolean, Dispatch<SetStateAction<boolean>>];
  systemTheme: [boolean, Dispatch<SetStateAction<boolean>>];
};
...
export function ThemeProvider({ children }: ThemeProviderProps): JSX.Element {
  ...
  const isSystemDarkModeInitial = useMemo(
    () => checkMediaProperty(PREFERS_COLOR_SCHEME),
    []
  );
  ...
  useEffect(() => {
    browserStorage.setItem(DARK_MODE, isUserDarkMode);
  }, [isUserDarkMode]);
